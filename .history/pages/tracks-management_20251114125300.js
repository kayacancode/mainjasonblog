...
export default function TracksManagement() {
    ...
    const handleUpdateTrack = async (e) => {
        ...
        try {
            ...
            // Check if the updated track would be in the top 10 by popularity
            // Only regenerate tracklist if it would be included
            const newTrackPopularity = cleanFormData.popularity || editingTrack?.popularity || 0;
            const weeksToRegenerate = new Set();
            
            // Check old week if it exists and is different from new week
            if (oldWeekStart && oldWeekStart !== newWeekStart) {
                const { data: oldWeekTracks } = await supabase
                    .from('tracks')
                    .select('*')
                ...
                if (oldWeekTracks) {
                    const top10Tracks = oldWeekTracks.slice(0, 10);
                    const minTop10Popularity = top10Tracks.length > 0
                        ? Math.min(...top10Tracks.map(t => t.popularity || 0))
                        : 0;
                    
                    // If removing this track would affect top 10, regenerate
                    const wasInTop10 = editingTrack && (editingTrack.popularity || 0) >= minTop10Popularity;
                    if (wasInTop10) {
            ...
            // Check new week
            if (newWeekStart) {
                const { data: newWeekTracks } = await supabase
                    .from('tracks')
                    .select('*')
                ...
                if (newWeekTracks) {
                    const top10Tracks = newWeekTracks.slice(0, 10);
                    const top10Popularities = top10Tracks.map(t => t.popularity || 0);
                    const minTop10Popularity = top10Popularities.length > 0
                        ? Math.min(...top10Popularities)
                        : 0;
                    
                    // Only regenerate if the updated track's popularity qualifies for top 10
                    if (newTrackPopularity >= minTop10Popularity || top10Tracks.length < 10) {
            ...
            if (weeksToRegenerate.size === 0) {
                console.log(`ℹ️ Updated track has popularity ${newTrackPopularity}, which doesn't affect top 10. Skipping tracklist regeneration.`);
            }
        } catch (error) {
