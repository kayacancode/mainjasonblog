...
export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            ...
            const originalFontconfig = process.env.FONTCONFIG_PATH;
            ...
            if (originalFontconfig) {
                process.env.FONTCONFIG_PATH = originalFontconfig;
            } else {
...
async function generateTracklistImage(tracks, weekStart) {
    ...
    try {
        // Suppress fontconfig warnings by setting environment variable
        process.env.FONTCONFIG_PATH = '/dev/null';
        
        let pngBuffer = await sharp(Buffer.from(svg), {
            density: 72, // Standard DPI
        })
        .png({ 
            quality: 95,
            compressionLevel: 9
