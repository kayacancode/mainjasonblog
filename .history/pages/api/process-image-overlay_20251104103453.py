...
Reuses the existing SpotifyNewMusicAutomation class
"""
...
# Add parent directory to path to import from spotify_api
script_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(script_dir)
sys.path.insert(0, parent_dir)

try:
    from pages.api.spotify_api.main import SpotifyNewMusicAutomation
except ImportError:
    # Try alternative import path
    try:
        import sys
        api_dir = os.path.join(os.path.dirname(script_dir), 'spotify_api')
...
def process_image_with_overlay(image_url, track_name, artist_name):
    ...
    try:
        ...
        try:
            ...
            # Create automation instance (credentials not needed for image processing)
            automation = SpotifyNewMusicAutomation('dummy', 'dummy')
            
            # Use the existing create_single_artist_image method
            # But we need to modify it to accept a local file path
            ...
            # Load font function (simplified)
            def load_font(size):
                try:
                    return ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", size, index=2)
                except:
                    try:
