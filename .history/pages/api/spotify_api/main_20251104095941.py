...
class SpotifyNewMusicAutomation:
    ...
    def create_single_artist_image(self, track: Dict, spotify_client, output_filename: str = "nmf_single_artist.png") -> str:
        ...
        try:
            ...
            iswt_text = "ISWT"
            iswt_font = load_font_prefer_helvetica(int(40 * size_multiplier), condensed=False)
            ...
            iswt_x = target_size[0] - iswt_width - 40  # 40px margin from right
            iswt_y = target_size[1] - iswt_height - 40  # 40px margin from bottom
            
            # Draw ISWT with white text and subtle shadow on overlay
            draw_overlay.text((iswt_x + 2, iswt_y + 2), iswt_text, fill=(0, 0, 0, 200), font=iswt_font)  # Shadow
    ...
    def create_tracklist_image(self, tracks: List[Dict], output_filename: str = "nmf_tracklist.png", custom_title: str = None) -> str:
        ...
        Args:
            ...
            custom_title: Optional custom title for the tracklist (defaults to "Top X Tracks")
        ...
        # Title section - use custom title if provided, otherwise default
        title = custom_title if custom_title else f"Top {len(sorted_tracks)} Tracks"
        ...
        iswt_text = "ISWT"
        iswt_font = load_font_prefer_helvetica(int(32 * size_multiplier), condensed=False)
        iswt_bbox = draw.textbbox((0, 0), iswt_text, font=iswt_font)
        iswt_width = iswt_bbox[2] - iswt_bbox[0]
        iswt_height = iswt_bbox[3] - iswt_bbox[1]
        iswt_x = canvas_width - iswt_width - 30  # 30px margin from right
        iswt_y = canvas_height - iswt_height - 30  # 30px margin from bottom
        
        # Draw ISWT with white text and subtle shadow
        draw.text((iswt_x + 2, iswt_y + 2), iswt_text, fill=(0, 0, 0, 180), font=iswt_font)  # Shadow
