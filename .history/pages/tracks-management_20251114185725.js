...
export default function TracksManagement() {
    ...
    const handleUpdateTrack = async (e) => {
        ...
        try {
            ...
            // Check each affected week to see if the updated track is/was in top 10
            for (const weekStart of weeksToCheck) {
                ...
                // Check if track was in top 10 before update (for old week)
                const wasInTop10 = weekStart === oldWeekStart && oldWeekStart !== newWeekStart
                    ? await isTrackInTop10(weekStart, editingTrack.id, oldPopularity)
                    : false;
                
                // Check if track is in top 10 after update (for new week)
                const isInTop10 = await isTrackInTop10(weekStart, editingTrack.id, newPopularity);
                
                // Also check if track name or artist changed (affects display)
                ...
                // Regenerate if:
                // 1. Track is/was in top 10, OR
                // 2. Track info (name/artist) changed (affects display even if not in top 10)
                if (wasInTop10 || isInTop10 || trackInfoChanged) {
                    console.log(`🔄 Track update affects top 10 or display. Regenerating tracklist for week ${weekStart}...`);
                    await regenerateTracklistForWeek(weekStart);
                } else {
                    console.log(`ℹ️ Updated track (popularity: ${newPopularity}) is not in top 10. Skipping regeneration for week ${weekStart}.`);
                }
            }
        } catch (error) {
