  ...
  const createDraft = async (e) => {
    ...
    try {
      if (router.query.postId) {
        const draftCollectionRef = collection(db, "drafts" ,router.query.postId);
        await updateDoc(draftCollectionRef,data)
      ...
      const draftCollectionRef = collection(db, "drafts");
      const { id } = await addDoc(draftCollectionRef, { ...data, createdAt: Date.now() });
      await imgUpload(id);
...
<div className="min-h-screen  items-center justify-center bg-[#303030]">
  ...
  onClick={handlePost}
>
  {loading ? "Loading" : (router.query.postId ? "Save" : "Post")}
</button>
<button
  type="submit"
  ...
  className="bg-[#F2EA6D] text-[#000] px-2.5 py-1.5 rounded"
>
  {loading ? "Loading" : (router.query.postId ? "Publish" : "Save as Draft")}
</button>
