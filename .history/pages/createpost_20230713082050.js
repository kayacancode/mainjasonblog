...
import { collection, doc, setDoc, addDoc, getDoc, updateDoc , getDocs,  query,
...
const Createpost = () => {
  ...
  const fetchDraftData = async () => {
    const docRef = doc(db, "drafts", router.query.postId);
    const docSnap = await getDoc(docRef);
  ...
  const createDraft = async (e) => {
    ...
    try {
      if (router.query.postId) {
        const draftDocRef = doc(db, "drafts", router.query.postId);
        await updateDoc(draftDocRef, data);
        ...
        await imgUpload(router.query.postId);
      ...
      const draftCollectionRef = collection(db, "drafts");
      const { id } = await addDoc(draftCollectionRef, {
        ...data,
        createdAt: Date.now(),
      });
      await imgUpload(id);
  ...
  const handleSaveDraft = (e) => {
    ...
    createDraft(e); // Call the saveDraft function for saving the draft
  };
...
<div className="min-h-screen  items-center justify-center bg-[#303030]">
  ...
  onClick={handlePost}
>
  {loading ? "Loading" : (router.query.postId ? "Publish" : "Post")}
</button>
<button
  type="submit"
  ...
  className="bg-[#F2EA6D] text-[#000] px-2.5 py-1.5 rounded"
>
  {loading ? "Loading" : (router.query.postId ? "Continue as draft" : "Save as Draft")}
</button>
