...
<svg xmlns="http://www.w3.org/2000/svg" width="${CANVAS_WIDTH}" height="${CANVAS_HEIGHT}" viewBox="0 0 ${CANVAS_WIDTH} ${CANVAS_HEIGHT}">`;
    ...
    let stackY = CANVAS_HEIGHT - margin - 160;
...
export async function generateSlide2(options) {
    ...
    const headerMargin = 40; // Horizontal margin inside header bar
    ...
    // Calculate dynamic font size for header to fit title on one line
    const headerText = title ? title.toUpperCase() : 'SUMMARY';
    const headerAvailableWidth = CANVAS_WIDTH - (headerMargin * 2);
    let headerFontSize = 42; // Start with default size
    const minHeaderFontSize = 20; // Minimum readable size
    
    // Find optimal font size for header
    while (headerFontSize > minHeaderFontSize) {
        const estimatedWidth = headerText.length * (headerFontSize * 0.55);
        if (estimatedWidth <= headerAvailableWidth) break;
        headerFontSize -= 2;
    }
...
<defs>
    <style type="text/css">
        <![CDATA[
            .header { font-family: 'Liberation Sans', sans-serif; font-weight: bold; font-size: ${headerFontSize}px; fill: ${PURE_WHITE}; }
    ...
    // Header text with dynamic font size (no truncation)
 ...
 * @param {string} options.subtitle - Subtitle for Slide 1 (default: 'Album Review')
...
export async function generateCarouselSlides(options) {
    const { postId, title, coverImageUrl, aiSummary, subtitle = 'Album Review', upload = true } = options;
