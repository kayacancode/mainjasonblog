...
export async function generateSlide1(options) {
    ...
    // Process image: resize to 1080x1080
...
<svg xmlns="http://www.w3.org/2000/svg" width="${CANVAS_WIDTH}" height="${CANVAS_HEIGHT}" viewBox="0 0 ${CANVAS_WIDTH} ${CANVAS_HEIGHT}">`;
    ...
    stackWords.forEach(({ text, color, fontSize }) => {
        svgContent += createTextSVG(text, CANVAS_WIDTH - margin - 50, stackY, fontSize, color, 'rgba(0, 0, 0, 0.5)', 2, 'end');
        ...
        return await sharp(buffer)
            .composite([
                { 
                    ...
                    left: Math.round(logoLeft - padding), 
                    top: Math.round(logoTop - padding) 
                },
                { 
                    input: resizedLogo, 
